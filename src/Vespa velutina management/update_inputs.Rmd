...
Managed_nests_fireman_in_database <- Managed_nests_fireman_in_database_raw %>% 
  mutate(Date_fireman = case_when(is.na(Date_fireman) ~ as.Date("2023-12-1"),
                   TRUE ~ as.Date(Date_fireman)))%>% #we zijn niet op de hoogte van datum dus wijzen voor zekerheid laatse mogelijke bestrijdingsdatum binnen seizoen aan
  group_by(ID_IAsset) %>%
