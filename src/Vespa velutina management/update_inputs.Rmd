...
dupli_nest_iasset <- dupli_nest_iasset %>% 
  left_join(data_nest_iasset %>% 
              select(id, observation_time) %>% 
              st_drop_geometry(), by = c("ID1" = "id")) %>% 
  left_join(data_nest_iasset %>% 
              select(id, observation_time) %>% 
              st_drop_geometry(), by = c("ID2" = "id")) 

dupli_missing_ID1 <- dupli_nest_iasset %>% 
  filter(is.na(observation_time.x))

dupli_missing_ID2 <- dupli_nest_iasset %>% 
  filter(is.na(observation_time.y))

dupli_missing_both <- dupli_nest_iasset %>% 
  filter(is.na(observation_time.x) & is.na(observation_time.y)) %>% 
...
data_nest_iasset <- data_nest_iasset %>% 
  filter(!id %in% dupli_nest_iasset$ID1) 
```
