...
data_nests_waarnemingen <- read_sheet("1gVrPeMj74pDkHd5oQJ8fmlMRAP5Dh85SManUzLI1eJQ",
                               sheet = "Sheet2",
                               col_types = "c")

# combineer iAsset en Waarnemingen observaties in 1 dataframe
data_nest_iasset_raw <- rbind(data_nest_iasset_raw,
...
Managed_nests_fireman_in_database <- read_delim("data/interim/Managed_nests_fireman_in_database.csv", 
                                      delim = ";", escape_double = FALSE, trim_ws = TRUE)

Managed_nests_fireman_not_in_database <- read_delim("data/interim/Managed_nests_fireman_not_in_database.csv", 
                                      delim = ";", escape_double = FALSE, trim_ws = TRUE) #TODO: toevoegen aan databank (worden nu enkel hier ingelezen)
```
...
```{r IASSET: data prep}
#correct coordinates
#TODO: nesten met coordinaten NA : provincie en gemeente onbekend ipv verwijderen uit data!
data_nest_iasset_raw <- data_nest_iasset_raw %>% 
...
data_nest_iasset_raw <- data_nest_iasset_raw %>% 
  ...
  mutate(datum_bestrijding = as.Date(datum_en_tijd_bestrijding))%>%#TODO: verwijder je hierdoor de tijd? Ook nog aan te passen bij oorspronkelijke data
  mutate(datum_bestrijding = case_when(insp_order%in%Managed_nests_fireman_in_database$ID_IAsset ~ Date_fireman,
