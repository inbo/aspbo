...
Vlaanderen <- prov %>%
  filter(NAAM %in% c('Oost-Vlaanderen', 
                     'West-Vlaanderen',
                     'Limburg',
                     'Vlaams-Brabant',
                     'Waals-Brabant'))
...
data_nest_final_Vlaanderen <- st_intersection(data_nest_final, Vlaanderen)
...
nests_per_month_per_year <- data_nest_final_Vlaanderen %>%
 ...
 write.csv2(aantal_nesten_per_maand_per_jaar_meta_path, row.names=FALSE)
```
