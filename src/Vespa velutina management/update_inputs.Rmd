...
dupli_nest_iasset <- dupli_nest_iasset%>%
  mutate(duplicate_to_remove = case_when(
    ID1 %in% dupli_missing_ID1 ~ ID2,
    ID2 %in% dupli_missing_ID2 ~ ID1,
    (ID1 %in% managed_nests$id) & !(ID2 %in% managed_nests$id) ~ ID1,
    (ID2 %in% managed_nests$id) & !(ID1 %in% managed_nests$id) ~ ID2,
                                       TRUE ~ ID2))
data_nest_iasset <- data_nest_iasset %>% 
