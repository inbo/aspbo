...
data_nest_iasset <- data_nest_iasset_raw %>% 
  ...
  mutate(result = case_when(resultaat == "succesvol behandeld" ~ "succesvol",
                             ...
                             (beheerd == "nee") & (bestrijding_gestart == "nee") ~ "niet behandeld",
...
if(!"id" %in% colnames(data_nest_iasset)){
  data_nest_iasset <- data_nest_iasset %>% 
...
```{r export nesten}
if(file.exists("./data/output/UAT_processing/Vespa_velutina_shape/nesten.geojson")){
  file.remove("./data/output/UAT_processing/Vespa_velutina_shape/nesten.geojson")
}

st_write(data_nest_final %>% filter(year >= 2017),
         "./data/output/UAT_processing/Vespa_velutina_shape/nesten.geojson",
         append = FALSE)
