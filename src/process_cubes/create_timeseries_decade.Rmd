...
```{r connect to bucket}
if(Sys.getenv("amiabot") != "yes"){
  message("Executor of the script is a human >> connecting to bucket")
  source("./src/connect_to_bucket.R")
  connect_to_bucket(bucket_name = UAT_bucket)
}else{
  message("Executor of the script is a bot >> testing credentials")
...
decade_max_input <- decade_min + 9

# Overwrite decade_max when larger than current year
if(decade_max_input > year(Sys.Date())){
  decade_max <- year(Sys.Date())
}else{
  decade_max <- decade_max_input
}

message("Calculating ", decade_min, "-", decade_max, " timeframe \n")
```
...
write_csv(df_ts, paste0("./cube_data/timeseries_", decade_min, "_", decade_max_input,".csv"))
```