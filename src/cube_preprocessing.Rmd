...
createTimeseries <- function(
  ...
  shapeData <- as.data.table(sf::st_drop_geometry(shapeData))
  regionCols <- c("CELLCODE", paste0("is", simpleCap(regions)))
  shapeData <- shapeData[, ..regionCols]
  
  # Chunk processing
  chunk_size <- 1e6  # Adjust chunk size based on your memory capacity
  num_chunks <- ceiling(nrow(df_ts) / chunk_size)
  
  timeseries_list <- lapply(1:num_chunks, function(i) {
    start_row <- (i - 1) * chunk_size + 1
    end_row <- min(i * chunk_size, nrow(df_ts))
    chunk <- df_ts[start_row:end_row]
    
    # Merge each chunk with shapeData
    merge(chunk, shapeData, by.x = "eea_cell_code", by.y = "CELLCODE", all.x = TRUE)
  })
  
  # Combine all chunks into one data.table
  timeseries <- rbindlist(timeseries_list)
  
  setkey(timeseries, taxonKey)
  
  # Write the result to S3 using Parquet format
