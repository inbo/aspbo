...
last_observed_taxon <- be_alientaxa_cube %>% 
  ...
  mutate(region = case_match(region,
                             "isFlanders" ~ "Vlaanderen",
                             "isWallonia" ~ "Wallonië",
                             "isBrussels" ~ "Brussels Hoofdstedelijk Gewest",
                             "isBelgium" ~ "België")) %>% 
  mutate(taxonKey = as.integer(taxonKey),
         last_observed_recalc = as.character(last_observed_recalc))

data_input_checklist_indicators <- alienSpecies::loadTabularData(type = "indicators") %>% 
...
# we expect the post update size to be larger than the pre update size
if(cube_post_update_size == cube_pre_update_size){
  warning("size unchanged")
}else{
  test_that("UPLOAD FAILED: size decreased",
            {
              expect_true(cube_post_update_size > cube_pre_update_size)
            })
}
